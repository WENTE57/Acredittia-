...
export default function Sidebar({ isOpen, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  const subtitulo = esAdmin ? "Administración" : user?.contrato?.nombre ?? user?.company?.nombre ?? "Mi Empresa";
  ...
  return (
    <>
      <aside className={`side ${isOpen ? 'mob-open' : ''}`}>
        {/* Logo */}
        <div className="slogo">
          <svg className="mark" viewBox="0 0 100 92" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <linearGradient id="cop1b" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#e3ad74"/><stop offset=".5" stopColor="#c98a4b"/><stop offset="1" stopColor="#8f5223"/></linearGradient>
              <linearGradient id="cop2b" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#f2cda0"/><stop offset="1" stopColor="#b06a30"/></linearGradient>
            </defs>
            <path d="M50 6 L92 80 L66 80 L50 42 L34 80 L8 80 Z" fill="url(#cop1b)"/>
            <path d="M50 6 L66 36 L50 44 Z" fill="url(#cop2b)"/>
            <path d="M50 6 L92 80 L80 80 L50 24 Z" fill="#6e3e18" opacity=".22"/>
          </svg>
          <div>
            <span className="wm">ACREDIT<span className="cyan">TIA</span></span>
        ...
        {/* User Info */}
        <div className="suser">
          <div className="su-av" id="su-av">{initial}</div>
        ...
        {/* Company Selector */}
        <div className="orgsw" onClick={() => alert('Cambiar de empresa — demo')}>
          <span className="orgic">⛏️</span>
          <span className="orgnm" id="dyn-orgname">{subtitulo}</span>
        ...
        <nav className="snav" id="snav">
          {esAdmin && (
            <Link href="/admin" className={`snavi ${path === "/admin" ? "active" : ""}`}>
              <span className="ico">🛡️</span>Panel admin
            </Link>
          )}
          {(!esAdmin || impersonando) && ITEMS.map((i) => (
            <Link key={i.href} href={i.href} className={`snavi ${path.startsWith(i.href) ? "active" : ""}`}>
              <span className="ico">{i.icono}</span>
              {i.nombre}
              {i.href === "/alertas" && noLeidas > 0 && <span className="nbadge">{noLeidas}</span>}
            </Link>
          ))}
        </nav>
        ...
        <div className="sbottom">
          <Link href="/config" className={`snavi ${path.startsWith("/config") ? "active" : ""}`}>
            <span className="ico">⚙️</span>Configuración
          </Link>
          <a className="snavi" onClick={() => alert('Centro de ayuda — próximamente')} style={{ cursor: "pointer" }}>
            <span className="ico">❓</span>Ayuda
          </a>
          <a className="snavi snavi-logout" onClick={salir} style={{ cursor: "pointer" }}>
            <span className="ico">🚪</span>Cerrar sesión
          </a>
        </div>
      </aside>
      
      {isOpen && <div className="mob-overlay" onClick={onClose} style={{ display: 'block' }}></div>}
    </>
  );
}
