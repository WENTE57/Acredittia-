...
const ESTADO_CLASE: Record<string, string> = {
  ...
  bloqueante: "bg-red-100 text-red-800",
...
export function Chip({ estado, texto }: { estado: string; texto?: string }) {
  return (
    <span
      className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold ${ESTADO_CLASE[estado] ?? "bg-slate-100 text-slate-600"}`}
    >
...
export function Kpi({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className={`mt-1 text-2xl font-bold ${color ?? "text-brand"}`}>
        {valor}
      </div>
...
export function Vacio({
  ...
  return (
    <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center">
      ...
      {cta && (
        <button onClick={onCta} className="btn-primary mt-4">
          {cta}
        </button>
      )}
    </div>
